...
use quote::quote;
use syn::{
    Data, DeriveInput, Field, Fields, Ident, LitStr, Meta, Token, Type,
    parse_macro_input,
...
fn parse_struct_filters(attrs: &[syn::Attribute]) -> syn::Result<Vec<MatchFilter>> {
    let mut out = Vec::new();
    for attr in attrs {
        if !attr.path().is_ident("journal") { continue; }
        let nested = attr.parse_args_with(
            Punctuated::<Meta, Token![,]>::parse_terminated
        )?;
        for meta in &nested {
            if let Meta::List(list) = meta {
                if list.path.is_ident("filter") {  // ← was "match"
                    let (ident, value) = list.parse_args_with(
                        |p: syn::parse::ParseStream| {
                            let i: Ident     = p.parse()?;
                            let _: Token![=] = p.parse()?;
                    ...
                    out.push(MatchFilter { field: ident.to_string(), value });
                }
            }
        }
    }
...
fn parse_field_meta(field: &Field) -> syn::Result<FieldMeta> {
    ...
    Ok(FieldMeta { journal_key, required })
}
...
fn named_fields(ast: &DeriveInput) -> syn::Result<&Punctuated<Field, Token![,]>> {
    match &ast.data {
        Data::Struct(s) => match &s.fields {
            ...
            _ => Err(syn::Error::new(ast.span(), "CrashEvent requires named fields")),
        },
        _ => Err(syn::Error::new(ast.span(), "CrashEvent can only be derived for structs")),
    }
}
...
fn is_option_string(ty: &Type) -> bool {
    ...
    if seg.ident != "Option" { return false; }
